...
const DetectionsList = () => {
  ...
  {[position].map((pos) => (
    <Marker key={pos[0]} position={pos} icon={icon}></Marker>
  ))}
</MapContainer>
  <h3>Bin Capacity</h3>
                  ...
                  {detection.image && (
                    <img
                      src={`data:image/png;base64,${
                        btoa(
                          // decode hex format
                          atob(detection.image.data.replace(/\\x([0-9A-F]{2})/gi,"$1")),
                          // OR decode escape format
                          unescape(detection.image.data.replace(/\\([0-7]{3})/g,function(match,g1){return String.fromCharCode(parseInt(g1,8))}))
                        )
                      }`}
                      alt="Detection"
                      height="100"
